...
export default class Github extends ApiBase {
  ...
  async refreshCopilotSession(): Promise<void> {
    if (this.copilotSession?.exp && this.copilotSession?.exp >= currentUnixTimestamp()) {
      return
    }
    ...
    const data = await this.request({
      ...
      headers: {
        ...
        "Editor-Plugin-Version": "helix-gpt/0.34",
        "Editor-Version": "helix-gpt/0.34,        
      }
    })
